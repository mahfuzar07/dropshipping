...
import DesignationForm from '@/components/pages/admin/workspace/designation/DesignationForm';
...
export default function AddDesignationDrawer() {
	...
	return (
		<Drawer open={isDrawerOpen} onOpenChange={(open) => !open && closeDrawer()} direction="right">
			<DrawerContent className="h-full !w-full md:min-w-xl flex flex-col bg-white">
				<button
				...
				<div className="h-full overflow-y-auto">
					<div className="border-b px-5 py-6 font-hanken text-right fixed w-full top-0 h-16 bg-dashboard-background flex- items-center text-lg uppercase text-dashboard-muted">
						<CardTitle>{isEdit ? 'Edit Designation' : 'Add Designation'}</CardTitle>
					</div>
